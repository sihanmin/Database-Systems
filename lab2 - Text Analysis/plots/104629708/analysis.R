...
time.data <- read.csv("time_data 0.2.csv", stringsAsFactors=FALSE)
...
state.data <- read.csv("state_data 0.2.csv", header=TRUE)
...
submission.data <- read.csv("submission_score 0.2.csv", quote="", header=TRUE)
...
comment.data <- read.csv("comment_score 0.2.csv", quote="", header=TRUE)
...
##################################################
# EXTRA PLOT: PARTICIPATION OVER TIME (TIME SERIES PLOT)
###################################################

# This assumes you have a CSV file called "participation_data.csv" with the columns:
# date (like 2018-08-01), Total
# You should use the FULL PATH to the file, just in case.

participation.data <- read.csv("participation_data.csv", stringsAsFactors=FALSE)
participation.data$date <- as.Date(participation.data$date)
# Fix a small data integrity issue from the data developer.
participation.data <- participation.data[participation.data$date != "2018-12-31", ]

# Get start and end of time series.
start <- min(participation.data$date)
end <- max(participation.data$date)

# Turn the data into a time series.
total.ts <- ts(participation.data$Total, start=start, end=end)

# Plot it
ts.plot(total.ts, col=c("black"),
gpars=list(xlab="Day", ylab="Participation", main="Participation in Discussion of Presidental Election Over Time"))
